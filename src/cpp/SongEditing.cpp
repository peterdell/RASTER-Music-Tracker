        ...
        if (songp[i] < 0) {
            ...
            while (songp[apos] < 0) {
                ...
                if (n >= 0) { //jump to another line
                    ...
                    if (!ok) { //has not found any tracks in this subsong yet
                        for (j = 0; j < g_tracks4_8; j++) {
                            if (m_song[apos][j] >= 0) { //if then found, this will be the beginning of the subsong
                                s.Format("%02X ", apos);
                                resultstr += s;
...
void CSong::ActiveInstrPrev() {
    g_Undo.Separator();
    int instr = (m_activeinstr - 1) & 0x3f;
...
BOOL CSong::SetBookmark() {
    if (m_songactiveline >= 0 && m_songactiveline < SONGLEN && m_trackactiveline >= 0 && m_trackactiveline < g_Tracks.GetMaxTrackLength() && m_speed >= 0) {
...
void CSong::InstrInfo(int instr, TInstrInfo* iinfo, int instrto) {
    ...
    if (iinfo) { //iinfo != NULL => set values
...
void CSong::TrackInfo(int track, TTrackInfo* tinfo) {
    ...
    if (tinfo) { //tinfo != NULL => set values
        tinfo->count = total;
        tinfo->lines = lines;
        for (ch = 0; ch < SONGTRACKS; ch++) {
            tinfo->usedincolumn[ch] = trackusedincolumn[ch];
        }
    } else { //tinfo == NULL => shows dialog
        CString s, s2;
        s.Format("Track: %02X\nUsing in song:\n", track);
        for (ch = 0; ch < g_tracks4_8; ch++) {
    ...
    while (!in.eof()) {
        ...
        if (strcmp(line, "MODULE]") == 0) {
            ...
            while (!in.eof()) {
                ...
                } else if (strcmp(line, "MAXTRACKLEN:") == 0) {
                ...
                } else if (strcmp(line, "MAINSPEED:") == 0) {
                    // Set the play speed: MAINSPEED: 01-FF
                    int v = Hexstr(value, 2);
                    if (v > 0) {
                        m_mainSpeed = v;
                    }
                } else if (strcmp(line, "INSTRSPEED:") == 0) {
...
// TrackUp()/TrackDown() only conditionally call Stop()/Play() inside
// "if (m_play && m_followplay)" branches, never taken here since nothing
// calls Play() on the instance first (m_play defaults to PLAY_STOP).
//
// PlayPressedTones()/InstrPaste() call into CAtariTrackerDriver, which
// only needs g_rmtinstr and CAtari::JSR() - itself just a delegate to the
// already-stubbed no-op C6502::JSR() (see AtariStub.cpp and
// AtariTrackerDriverCore.cpp's header comment).
//
// SongInsertCopyOrCloneOfSongLines()/TracksOrderChange() were also
// considered for this batch but turned out to instantiate real MFC
// dialogs (CInsertCopyOrCloneOfSongLinesDlg/CSongTracksOrderDlg) - the
// same hazard class as InstrChange()'s CInstrumentChangeDlg, not just a
// guard-only MessageBox - so they stay deferred alongside InstrChange().
...
// Play()/Stop()/PlayBeat()/PlayVBI() only call
// g_SongTimer.WaitForTimerRoutineProcessed() (a confirmed-safe no-op as
// long as SetTimer() is never called, see SongEditingStub.cpp's header
// comment on g_SongTimer), g_Atari.Init() (delegates to the already-
// stubbed no-op C6502::Init()), and g_AtariTrackerDriver (already safe,
// see AtariTrackerDriverCore.cpp). ChangeTimer()/StopTimer()/
// TimerRoutine() are NOT included here - TimerRoutine() also calls
...
BOOL CSong::Play(PlayMode mode, BOOL follow, int special) {
    ...
    case PLAY_BLOCK: //only in the block
        if (!g_TrackClipboard.IsBlockSelected()) { //no block is selected, so the track plays
            mode = PLAY_TRACK;
            goto Play3;
        } else {
            int bfro, bto;
    ...
    for (t = 0; t < g_tracks4_8; t++) {
        ...
        if (m_trackplayline >= len) {
            if (go >= 0) {
                ...
                if (m_play == PLAY_BLOCK || m_play == PLAY_STOP) {
                    note[t] = -1;
                    instr[t] = -1;
                    vol[t] = -1;
                    continue;
    ...
    for (t = 0; t < g_tracks4_8; t++) {
        ...
        if (v >= 0 && v < 16) {
            if (n >= 0 && n < CNotes::NOTESNUM /*&& i>=0 && i<INSTRSNUM*/) { // adjustment for routine compatibility
                if (i < 0 || i >= INSTRSNUM) {
                    i = 255;
                } // adjustment for routine compatibility
                g_AtariTrackerDriver->SetTrackNoteInstrumentVolume(t, n, i, v);
            } else {
                g_AtariTrackerDriver->SetTrackVolume(t, v);
            }
...
BOOL CSong::PlayVBI() {
    ...
    if (m_play == PLAY_BLOCK && m_trackplayline > m_trackplayblockend) {
        m_trackplayline = m_trackplayblockstart;
    }
    ...
    if (m_speeda == m_speed && m_followplay) { //playing and following the player
        ...
        //Quantization
        if (m_quantization_note >= 0 && m_quantization_note < CNotes::NOTESNUM && m_quantization_instr >= 0 && m_quantization_instr < INSTRSNUM) {
        ...
        } else if (m_quantization_note == -2) { //Special case (midi NoteOFF)
