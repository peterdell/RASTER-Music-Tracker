...
BOOL CSong::FileOpen(const char* filename, BOOL warnOfUnsavedChanges) {
    ...
    if (!g_lastLoadPath_Songs.IsEmpty()) {
        ...
        if (!g_defaultSongsPath.IsEmpty()) {
            dlg.m_ofn.lpstrInitialDir = g_defaultSongsPath;
        }
    }

    if (GetIOType() == SongIOType::RMT) {
        dlg.m_ofn.nFilterIndex = FILE_LOADSAVE::RMT;
    }
    if (GetIOType() == SongIOType::TXT) {
        dlg.m_ofn.nFilterIndex = FILE_LOADSAVE::TXT;
    }
    if (GetIOType() == SongIOType::RMW) {
        dlg.m_ofn.nFilterIndex = FILE_LOADSAVE::RMW;
    }
...
void CSong::FileImport() {
    ...
    if (l_lastImportTypeIndex >= 0) {
        dlg.m_ofn.nFilterIndex = l_lastImportTypeIndex;
    } // Restore the last imported file type
