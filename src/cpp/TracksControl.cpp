...
void CTracksControl::DrawTrackLine(const CTracks& tracks, int col, int x, int y, int tr, const int line, int aline, int cactview, int pline, BOOL isactive, int acu, int oob, Notation notation) {
    char s[16] = " \x8\x8\x8 \x8\x8 \x8\x8 \x8\x8\x8";
    int len = -1, last = -1, go = -1;
    auto color = TextColor::WHITE;
    int n, xline;

    if (auto tt = tracks.GetConstTrack(tr)) {
        strcpy(s, " --- -- -- ---");
