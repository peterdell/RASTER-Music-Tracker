...
// Every CTrackClipboard method except BlockEffect() (which instantiates a
// real CEffectsDlg, an MFC dialog) only touches g_Tracks/g_Song (both
// confirmed cheap - see TracksTests.cpp/SongTests.cpp) plus the simple
// ClearStatusBar()/SetStatusBarText() helpers (which just no-op or
// OutputDebugString when there's no real status bar window - see
// GuiHelpers.cpp). Kept separate from Clipboard.cpp so this half doesn't
// need EffectsDlg.h's dialog or GuiHelpers.h's Global.h dependency - same
// pattern as Tuning.cpp/TuningTables.cpp, Tracks.cpp/TracksEdit.cpp,
// Instruments.cpp/InstrumentsCore.cpp+InstrumentsAtaFormat.cpp, and
// Song.cpp/SongCore.cpp+SongEditing.cpp.
