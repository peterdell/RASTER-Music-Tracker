...
// CSongExporter::StrToAtariVideo/BruteforceOptimalLZSS/ExportXEX_LZSS(xexFile
// overload) split from SongExporter.cpp - unlike ExportSAP_R/ExportSAP_B_LZSS
// (which delegate to the already dialog-free CSAPFileExporter class), this
// overload of ExportXEX_LZSS IS the dialog-independent real work itself, and
// StrToAtariVideo/BruteforceOptimalLZSS are its own private static helpers -
...
// CSongExporter::ExportLZSS()/ExportCompactLZSS() moved here from
// SongExporter.cpp, same reason as the methods above: neither shows a
// dialog (so there was no dialog-wrapper/*Apply() split to make - they're
