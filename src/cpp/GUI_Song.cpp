...
BOOL CSong::InfoKey(int vk, int shift, int control) {
    ...
    if (m_infoact == EditArea::NAME) {
        ...
        if (EditText(vk, shift, control, m_songname, m_songnamecur, SONG_NAME_MAX_LEN)) {
            ...
            if ((BOOL)control != (BOOL)g_keyboard_swapenter) { //control+Enter => plays a whole line (all tracks)
                ...
                for (int i = 0; i < GetTracks(); i++) {
                    if (i != m_trackactivecol) {
                        ...
                        } else if (vol >= 0) { //there is no note, but is there a separate volume?
...
TrackKeyOk:
    ...
    case VK_UP:
        if (control && shift) {
            if (!g_TrackClipboard.IsBlockSelected()) { //if no block is selected, make a block at the current location
    ...
    case VK_A:
        if (g_TrackClipboard.IsBlockSelected() && shift && control) { //Shift+control+A
            //switch ALL / no ALL
