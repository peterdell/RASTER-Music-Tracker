...
void CSongUI::DrawVolumeAnalyzer() {
    ...
    if (!stereo && g_active_ti == Part::PART_INSTRUMENTS && g_width > MINIMAL_WIDTH_INSTRUMENTS - 220) {
    ...
    if ((g_width < MINIMAL_WIDTH_TRACKS && g_active_ti == Part::PART_TRACKS) || (g_width < MINIMAL_WIDTH_INSTRUMENTS && g_active_ti == Part::PART_INSTRUMENTS)) {
    ...
    if (g_active_ti == Part::PART_TRACKS) { // bigger look for track edit mode
        ...
        for (int channelNr = 0; channelNr < m_song->GetTracks(); channelNr++) {
            ...
            if (g_ChannelControl.IsChannelOn(channelNr) && ((skctl1 == 0x8b && channelNr == 0) || (skctl2 == 0x8b && channelNr == 4))) {
                acol = CRGBColor::TWO_TONE;
            }
        ...
        if (viewPokeyRegisters) {
            ...
            if (stereo) {
                canvasXY->NumberMiniXY(memory[0xd218], ANALYZER_X + 23 + 5 * 8 * 16 + 80, ANALYZER_Y - 8);
            } // Stereo
            ...
            if (a & 0x04) {
                col[2] = CRGBColor::COL_BLOCK;
                DrawInstrumentHook(ANALYZER2_X, ANALYZER_Y, 0, 2, yUp);
                yUp -= 2;
            } // High pass filter on channel 1, clocked by channel 3
            if (a & 0x02) {
                col[3] = CRGBColor::COL_BLOCK;
                DrawInstrumentHook(ANALYZER2_X, ANALYZER_Y, 1, 3, yUp);
                yUp -= 2;
            } // High pass filter on channel 3, clocked by channel 4
            if (a & 0x10) {
                col[0] = CRGBColor::COL_BLOCK;
                DrawInstrumentHook(ANALYZER2_X, ANALYZER_Y, 0, 1, yUp);
                yUp -= 2;
            } // Join channels 1 + 2 (16 bit)
            if (a & 0x08) {
                col[2] = CRGBColor::COL_BLOCK;
                DrawInstrumentHook(ANALYZER2_X, ANALYZER_Y, 2, 3, yUp);
                yUp -= 2;
            } // Join channels 3 + 4 (16 bit)
            ...
            if (a & 0x04) {
                col[2 + 4] = CRGBColor::COL_BLOCK;
                DrawInstrumentHook(ANALYZER2_X, ANALYZER_Y, 0 + 4, 2 + 4, yUp);
                yUp -= 2;
            } // High pass filter on channel 5 clocked by channel 7
            if (a & 0x02) {
                col[3 + 4] = CRGBColor::COL_BLOCK;
                DrawInstrumentHook(ANALYZER2_X, ANALYZER_Y, 1 + 4, 3 + 4, yUp);
                yUp -= 2;
            } // High pass filter on channel 7, clocked by channel 8
            if (a & 0x10) {
                col[0 + 4] = CRGBColor::COL_BLOCK;
                DrawInstrumentHook(ANALYZER2_X, ANALYZER_Y, 0 + 4, 1 + 4, yUp);
                yUp -= 2;
            } // Join channels 5 + 6 (16 bit)
            if (a & 0x08) {
                col[2 + 4] = CRGBColor::COL_BLOCK;
                DrawInstrumentHook(ANALYZER2_X, ANALYZER_Y, 2 + 4, 3 + 4, yUp);
                yUp -= 2;
            } // Join channels 7 + 8 (16 bit)
            ...
            for (int channelNr = 0; channelNr < g_tracks4_8; channelNr++) {
                ...
                // Draw the volume bar in the selected color
                if (vol) {
                    canvasXY->FillSolidRect(ANALYZER2_X + channelNr * 3 * 8 + (15 - vol) * ANALYZER2_S / 2, ANALYZER2_Y, vol * ANALYZER2_S, ANALYZER2_H, acol);
                }
            }
        }
        if (DEBUG_POKEY) {
...
void CSongUI::DrawSong() {
    ...
    if (!m_song->IsStereo() && g_active_ti == Part::PART_INSTRUMENTS && g_width > MINIMAL_WIDTH_INSTRUMENTS - 220) {
        INSTRUMENT_OFFSET = 260;
    }
    ...
    if (active_smooth) {
        ...
        if (!pattern_len) {
            pattern_len = g_Tracks.GetMaxTrackLength(); //fallback to whatever is in memory instead if the value returned is invalid
        }
    ...
    for (i = 0; i < linescount + active_smooth * 2; i++, y += 16) {
        ...
        if ((j = m_song->m_songgo[line]) >= 0) { //there is a GO to line
            ...
            if (line == m_song->m_songactiveline) {
                if (IsProveMode()) {
                    color = (g_activepart == Part::PART_SONG) ? LogicalTextColor::SELECTED_PROVE : TextColor::BLUE;
                } else {
                    color = (g_activepart == Part::PART_SONG) ? LogicalTextColor::SELECTED : TextColor::RED;
                }
            }
            szBuffer[0] = CharH4(j);
            szBuffer[1] = CharL4(j);
            ...
            for (j = 0, k = 32; j < g_tracks4_8; j++, k += 24) {
                if ((t = m_song->m_song[line][j]) >= 0) {
                    ...
                    szBuffer[0] = szBuffer[1] = '-'; // No track here so draw "--"
                }
    ...
    for (i = 0; i < 4; i++, k += 24) {
        ...
        if (g_ChannelControl.IsChannelOn(i)) {
            if (m_song->m_trackactivecol == i) {
                color = IsProveMode() ? TextColor::BLUE : TextColor::RED; //active channel highlight
            } else {
                color = TextColor::WHITE; //normal channel
            }
        } else {
            color = TextColor::GRAY; //switched off channels are in gray
        }
...
void CSongUI::DrawTracks() {
    ...
    if (m_song->SongGetGo() >= 0) { //it's a GOTO line, it won't draw tracks
        ...
        if (IsProveMode()) {
            color = (g_activepart == Part::PART_TRACKS) ? LogicalTextColor::SELECTED_PROVE : TextColor::BLUE;
        } else {
            color = (g_activepart == Part::PART_TRACKS) ? LogicalTextColor::SELECTED : TextColor::RED;
        }
    ...
    if (smooth_y > 8 || smooth_y < -8) {
        active_smooth = smooth_y = 0; //prevents going out of bounds
    }
    ...
    for (i = 0; i < g_tracklines + active_smooth * 2; i++, y += 16) {
        ...
        if (line % g_trackLineSecondaryHighlight == 0) {
            color = TextColor::GREEN;
        }
        if (line % g_trackLinePrimaryHighlight == 0) {
    ...
    if (g_view.debugDisplay) {
        ...
        for (int i = 0; i < g_width / width; i++) {
            switch (i) {
            case 0:
                d.Format("GW=%04d", g_width);
                break;
            case 1:
                d.Format("GH=%04d", g_height);
                break;
            case 2:
                d.Format("PX=%04d", g_mouse.pointX);
                break;
            case 3:
                d.Format("PY=%04d", g_mouse.pointY);
                break;
            case 4:
                d.Format("MB=%02d", g_mouse.button);
                break;
            case 5:
                d.Format("CA=%02d", g_cursoractview);
                break;
            case 6:
                d.Format("TA=%02d", m_song->m_trackactiveline);
                break;
            case 7:
                d.Format("DY=%02d", g_mouse.pointY / 16);
                break;
            case 8:
                d.Format("GTL=%02d", g_tracklines);
                break;
            case 9:
                d.Format("OL=%02d", g_tracklines / 2);
                break;
            case 10:
                d.Format("VK=%c %02X", (char)LOWORD(MapVirtualKeyEx(g_lastKeyPressed, MAPVK_VK_TO_CHAR, NULL)), g_lastKeyPressed);
                break;
            case 11:
                d.Format("MO=%s%s", (g_shiftkey ? "S" : " "), (g_controlkey ? "C" : " "));
                break;
            case 12:
                d.Format("WD=%02d", g_mouse.wheelDelta);
                break;
            default:
                continue;
            }
...
void CSongUI::DrawInfo() {
    ...
    if (flag & IF_AUDCTL) {
        int audctl = g_Instruments.GetParameter(m_song->m_activeinstr, PAR_AUDCTL_15KHZ) | g_Instruments.GetParameter(m_song->m_activeinstr, PAR_AUDCTL_HPF_CH2) << 1 | g_Instruments.GetParameter(m_song->m_activeinstr, PAR_AUDCTL_HPF_CH1) << 2 | g_Instruments.GetParameter(m_song->m_activeinstr, PAR_AUDCTL_JOIN_3_4) << 3 | g_Instruments.GetParameter(m_song->m_activeinstr, PAR_AUDCTL_JOIN_1_2) << 4 | g_Instruments.GetParameter(m_song->m_activeinstr, PAR_AUDCTL_179_CH3) << 5 | g_Instruments.GetParameter(m_song->m_activeinstr, PAR_AUDCTL_179_CH1) << 6 | g_Instruments.GetParameter(m_song->m_activeinstr, PAR_AUDCTL_POLY9) << 7;
