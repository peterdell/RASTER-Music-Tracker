...
void CSong::MidiEvent(DWORD dwParam) {
    ...
    if (chn == 15 || chn == 9) {
        ...
        if (cmd == 0x90 && chn == 9 && IsEditMode(EditMode::MIDI_CH15_MODE)) { //drumpads used to control the POKEY registers
            switch (pr1) {
        ...
        if (chn == 15 && !IsEditMode(EditMode::MIDI_CH15_MODE)) {
            ...
            if (cmd == 0x90) {
                ...
                if (pr2 == 0) {
                    ...
                    vol = 0; //keyoff
                } else if (g_Midi.m_TouchResponse) {
                ...
                if (note >= 0 && note < CNotes::NOTESNUM) { //only within this range
                    ...
                    if (vol > 0) {
                        ...
                        else if (TrackSetNoteInstrVol(note, m_activeinstr, vol)) {
                            BLOCKDESELECT();
    ...
    if (chn == 0) {
        ...
        if (cmd == 0x90) {
            ...
            if (note >= 0 && note < CNotes::NOTESNUM) { //only within this range
                ...
                Prove_midi:
                    SetPlayPressedTonesTNIV(m_trackactivecol, note, m_activeinstr, vol);
                    if ((IsEditMode(EditMode::JAM_STEREO_MODE) || g_controlkey) && IsStereo()) { //with control or in prove2 => stereo test
                        SetPlayPressedTonesTNIV((m_trackactivecol + 4) & 0x07, note, m_activeinstr, vol);
                    }
                }
            }
