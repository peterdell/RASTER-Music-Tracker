...
void CInstruments::DrawInstrument(int instrNr) {
    ...
    canvasXY->TextXY(szBuffer, InstrumentGUIPosition::X, InstrumentGUIPosition::Y, TextColor::WHITE);

    int size = (t->parameters[PAR_ENV_LENGTH] + 1) * 3 + (t->parameters[PAR_TBL_LENGTH] + 1) + 12;
    sprintf(szBuffer, "(SIZE %u BYTES)", size);
    canvasXY->TextMiniXY(szBuffer, InstrumentGUIPosition::X + 14 * 8, InstrumentGUIPosition::Y + 5);

    DrawName(instrNr);

    // Draw some headings
    canvasXY->TextMiniXY("EFFECT", InstrumentGUIPosition::PARAM_X, InstrumentGUIPosition::PARAM_Y + 1 * 16 + 8);
    canvasXY->TextMiniXY("AUDCTL", InstrumentGUIPosition::PARAM_X + 0 * 8, InstrumentGUIPosition::PARAM_Y + 5 * 16 + 8);
    canvasXY->TextMiniXY("ENVELOPE", InstrumentGUIPosition::PARAM_X + 15 * 8, InstrumentGUIPosition::PARAM_Y + 16 + 8);
    canvasXY->TextMiniXY("TABLE", InstrumentGUIPosition::PARAM_X + 15 * 8, InstrumentGUIPosition::PARAM_Y + 8 * 16 + 8);
    ...
    if (t->activeEditSection == InstrumentSection::ENVELOPE) {
        ...
        canvasXY->TextXY(szBuffer, InstrumentGUIPosition::ENV_X + 2 * 8, InstrumentGUIPosition::ENV_Y + 5 * 16, TextColor::GRAY);
    }
    ...
    canvasXY->IconMiniXY(i, shpar[PAR_TBL_TYPE].x + 8 * 8 + 2, shpar[PAR_TBL_TYPE].y + 7);

    //inscription at the bottom of TABLE
    canvasXY->TextMiniXY((i == INSTRUMENT_TABLE_OF_NOTES) ? "TABLE OF NOTES" : "TABLE OF FREQS", InstrumentGUIPosition::TABLE_X, InstrumentGUIPosition::TABLE_Y - 8);

    //TABLE MODE icon (Set or Add)
    i = (t->parameters[PAR_TBL_MODE] == 0) ? INSTRUMENT_TABLE_MODE_SET : INSTRUMENT_TABLE_MODE_ADD;
    canvasXY->IconMiniXY(i, shpar[PAR_TBL_MODE].x + 8 * 8 + 2, shpar[PAR_TBL_MODE].y + 7);
    ...
    if (go < len) {
        ...
        canvasXY->TextXY(szBuffer, InstrumentGUIPosition::ENV_X + 12 * 8 + len * 8, InstrumentGUIPosition::ENV_Y + 7 * 16, TextColor::WHITE);
    ...
    if (len == 0) {
        canvasXY->TextXY("\x18", InstrumentGUIPosition::TABLE_X + 4, InstrumentGUIPosition::TABLE_Y + 8 + 16, TextColor::WHITE);
    } else {
        canvasXY->TextXY("\x19", InstrumentGUIPosition::TABLE_X + go * 8 * 3, InstrumentGUIPosition::TABLE_Y + 8 + 16, TextColor::WHITE);
        canvasXY->TextXY("\x1a", InstrumentGUIPosition::TABLE_X + 8 + len * 8 * 3, InstrumentGUIPosition::TABLE_Y + 8 + 16, TextColor::WHITE);
    }

    //boundaries of all parts of the instrument
    /*
    CBrush br(RGB(112,112,112));
    CCanvasXY->FrameRect(CRect(InstrumentGUIPosition::X-2,InstrumentGUIPosition::Y-2,InstrumentGUIPosition::X+38*8+4,InstrumentGUIPosition::Y+2*16+2),&br);
    CCanvasXY->FrameRect(CRect(InstrumentGUIPosition::PX-2,InstrumentGUIPosition::PY+16-2,InstrumentGUIPosition::PX+29*8,InstrumentGUIPosition::PY+15*16+4),&br);
    CCanvasXY->FrameRect(CRect(InstrumentGUIPosition::EX,InstrumentGUIPosition::EY-2*16-2,InstrumentGUIPosition::EX+48*8,InstrumentGUIPosition::EY+15*16+2),&br);
...
#define HORIZONTALLINE                                                                               \
    {                                                                                                \
        g_mem_dc->MoveTo(InstrumentGUIPosition::HELP_X, InstrumentGUIPosition::HELP_Y - 1);          \
        g_mem_dc->LineTo(InstrumentGUIPosition::HELP_X + 93 * 8, InstrumentGUIPosition::HELP_Y - 1); \
    }
    ...
    if (t->activeEditSection == InstrumentSection::ENVELOPE) { // is the cursor on the envelope?
        ...
        case EnvelopeParameter::DISTORTION: {
            ...
            canvasXY->TextXY(hs, InstrumentGUIPosition::HELP_X, InstrumentGUIPosition::HELP_Y, TextColor::GRAY);
            //HORIZONTALLINE;
        } break;
        ...
        case EnvelopeParameter::X:
        case EnvelopeParameter::Y: {
...
void CInstruments::DrawName(int instrNr) {
    ...
    int cursorPos = -1;
    auto color = TextColor::TURQUOISE;
    ...
    canvasXY->TextXY("NAME:", InstrumentGUIPosition::X, InstrumentGUIPosition::Y + 16, TextColor::WHITE); // Draw the title
    canvasXY->TextXYSelN(ptrName, cursorPos, InstrumentGUIPosition::X + 6 * 8, InstrumentGUIPosition::Y + 16, color); // Draw the name and highlight the cursor position
}
...
BOOL CInstruments::GetGUIArea(int instrNr, InstrumentGUIZone zone, CRect& rect) {
    ...
    case InstrumentGUIZone::ENVELOPE_LEFT_ENVELOPE:
        ...
        rect.SetRect(InstrumentGUIPosition::ENV_X + 12 * 8, InstrumentGUIPosition::ENV_Y + 3 * 16 + 4, InstrumentGUIPosition::ENV_X + 12 * 8 + len * 8, InstrumentGUIPosition::ENV_Y + 3 * 16 + 4 + 4 * 16);
        return TRUE;
    ...
    case InstrumentGUIZone::ENVELOPE_RIGHT_ENVELOPE:
        ...
        rect.SetRect(InstrumentGUIPosition::ENV_X + 12 * 8, InstrumentGUIPosition::ENV_Y - 2 * 16 + 4, InstrumentGUIPosition::ENV_X + 12 * 8 + len * 8, InstrumentGUIPosition::ENV_Y - 2 * 16 + 4 + 4 * 16);
        return TRUE;

    case InstrumentGUIZone::ENVELOPE_PARAM_TABLE:
        //envelope area large table
        rect.SetRect(InstrumentGUIPosition::ENV_X + 12 * 8, InstrumentGUIPosition::ENV_Y + 3 * 16 + 0 + 5 * 16, InstrumentGUIPosition::ENV_X + 12 * 8 + len * 8, InstrumentGUIPosition::ENV_Y + 3 * 16 + 0 + 5 * 16 + 7 * 16);
        return TRUE;
    ...
    case InstrumentGUIZone::ENVELOPE_RIGHT_VOL_NUMS:
        ...
        rect.SetRect(InstrumentGUIPosition::ENV_X + 12 * 8, InstrumentGUIPosition::ENV_Y - 2 * 16 + 0 + 4 * 16, InstrumentGUIPosition::ENV_X + 12 * 8 + len * 8, InstrumentGUIPosition::ENV_Y - 2 * 16 + 0 + 4 * 16 + 16);
        return TRUE;

    case InstrumentGUIZone::NOTE_TABLE:
        //instrument table line
        rect.SetRect(InstrumentGUIPosition::TABLE_X, InstrumentGUIPosition::TABLE_Y + 8, InstrumentGUIPosition::TABLE_X + tabl * 24 - 8, InstrumentGUIPosition::TABLE_Y + 8 + 16);
        return TRUE;

    case InstrumentGUIZone::INSTRUMENT_NAME:
        //instrument name
        rect.SetRect(InstrumentGUIPosition::PARAM_X, InstrumentGUIPosition::PARAM_Y - 16, InstrumentGUIPosition::PARAM_X + 6 * 8 + INSTRUMENT_NAME_MAX_LEN * 8, InstrumentGUIPosition::PARAM_Y + 0);
        return TRUE;

    case InstrumentGUIZone::PARAMETERS:
        //instrument parameters
        rect.SetRect(InstrumentGUIPosition::PARAM_X, InstrumentGUIPosition::PARAM_Y + 32, InstrumentGUIPosition::PARAM_X + 26 * 8, InstrumentGUIPosition::PARAM_Y + 32 + 12 * 16);
        return TRUE;

    case InstrumentGUIZone::INSTRUMENT_NUMBER_DLG:
        //instrument number
        rect.SetRect(InstrumentGUIPosition::X, InstrumentGUIPosition::Y, InstrumentGUIPosition::X + 13 * 8, InstrumentGUIPosition::Y + 16);
        return TRUE;

    case InstrumentGUIZone::LEN_AND_GOTO_ARROWS:
        //envelope area under the left (lower) volume curve
        rect.SetRect(InstrumentGUIPosition::ENV_X + 12 * 8, InstrumentGUIPosition::ENV_Y + 3 * 16 + 0 + 4 * 16, InstrumentGUIPosition::ENV_X + 12 * 8 + ENVELOPE_MAX_COLUMNS * 8, InstrumentGUIPosition::ENV_Y + 3 * 16 + 0 + 4 * 16 + 16);
        return TRUE;

    case InstrumentGUIZone::NOTE_TBL_LEN_AND_GOTO:
        //instrument table + 1 line below parameter table
        rect.SetRect(InstrumentGUIPosition::TABLE_X, InstrumentGUIPosition::TABLE_Y + 8 + 1 * 16, InstrumentGUIPosition::TABLE_X + NOTE_TABLE_MAX_LEN * 24 - 8, InstrumentGUIPosition::TABLE_Y + 8 + 2 * 16);
        return TRUE;
    }
    return FALSE;
}
...
void CInstruments::DrawEnv(int e, int it) {
    ...
    TextColor color;
    int x = InstrumentGUIPosition::ENV_X + 12 * 8 + e * 8;
    char s[2], a;
    s[1] = 0;
    ...
    COLORREF fillColor = (in->envelope[e][EnvelopeParameter::COMMAND] == 0x07 && in->envelope[e][EnvelopeParameter::X] == 0x0f && in->envelope[e][EnvelopeParameter::Y] == 0x0f) ? RGB(128, 255, 255) : RGB(255, 255, 255);
...
void CInstruments::DrawNoteTableValue(int noteIdx, int instrNr) {
    ...
    szBuffer[2] = 0;
    canvasXY->TextMiniXY(szBuffer, InstrumentGUIPosition::TABLE_X + noteIdx * 24, InstrumentGUIPosition::TABLE_Y);
    ...
    canvasXY->TextXY(szBuffer, InstrumentGUIPosition::TABLE_X + noteIdx * 24, InstrumentGUIPosition::TABLE_Y + 8, color);
}