...
class CConvertTracks {
  public:
    CConvertTracks();
    void SetRMTTracks(CTracks* tracks) {
        m_ctracks = tracks;
    };
    int Init();
    TSourceTrack* GetSTrack(int t) {
        return (t >= 0 && t < TRACKSNUM) ? &m_strack[t] : (TSourceTrack*)NULL;
    };
    TInstrumentMark* GetIMark(int instr) {
        return (instr >= 0 && instr < INSTRSNUM) ? &m_imark[instr] : (TInstrumentMark*)NULL;
    };
...
int CConvertTracks::MakeOrFindTrackShiftLR(int from, int shift, BYTE lr) {
    ...
    for (i = 0; i < ts->len; i++) {
        ...
        if (note >= 0) {
            ...
            if (note >= CNotes::NOTESNUM) {
                note = CNotes::NOTESNUM - 1;
            }
        }
        td->note[i] = note;
            ...
            */
            ...
            if (!nobytable && !tableu) {
                if (hn == 1 && (dn > 2 || vibspe > 0)) { //(dn>=4 || vibspe>0) )
                    ...
                    vpt = 1; //successful
                } else if (hn == 2 && (dn > 2 || vibspe > 0)) { //(dn>=4 || vibspe>0))
                    if (posuntable > NOTE_TABLE_MAX_LEN - 4) {
                        posuntable = NOTE_TABLE_MAX_LEN - 4; //did not give what is possible according to the delay
                    }
                    ...
                    vpt = 1; //successful
                } else if (hn == 3 && (dn > 2 || vibspe > 0)) { //(dn>=4 || vibspe>0))
                    if (posuntable > NOTE_TABLE_MAX_LEN - 4) {
                        posuntable = NOTE_TABLE_MAX_LEN - 4; //did not give what is possible according to the delay
                    }
...
// std::istream& directly alongside the parsed header. The six guard-only
// MessageBox calls in the original (too many tracks/songlines, a sample
// seek/read failure, a module-length mismatch) are kept exactly as they
// were - same "avoidable with valid test data" treatment already
// established throughout this effort (LoadRMT, MakeModule, ImportTMC's own
// corrupted-file guard) - except the three *header*-validation guards
// (bad header length, unsupported identification, truncated file), which
// happen before any dialog can be shown, so ImportMODParseHeader() reports
// them via a plain error code instead (see TImportMODHeader) and the
// wrapper (ImportMOD() in IO_Importer.cpp) reconstructs their exact
// original, differently-worded messages.
...
bool CSong::ImportMODParseHeader(std::istream& in, TImportMODHeader& header) {
    ...
    in.read((char*)&header.head, 1084);
    int flen = (int)in.gcount();

    header.head[1084] = 0; //finished behind the header
    ...
    header.modulelength = (int)in.tellg();
    ...
    header.patternsize = header.chnls * 256;
    ...
    in.read((char*)header.mem.data(), memlen); //load module
    flen = (int)in.gcount();
    if (flen != memlen) {
        header.errorCode = 3;
        return false;
...
void CSong::ImportMODApply(std::istream& in, const TImportMODHeader& header, int rmttype, const BYTE trackorder[8], BOOL x_shiftdownoctave, BOOL x_portamento, BOOL x_fullvolumerange, BOOL x_volumeincrease, BOOL x_decreaseinstrument, BOOL x_optimizeloops, BOOL x_truncateunusedparts, TImportMODResult& result) {
    const BYTE* mem = header.mem.data();
    int chnls = header.chnls;
    int modsamples = header.modsamples;
    int song = header.song;
    ...
    int modulelength = header.modulelength;
    ...
    for (i = 1; i <= modsamples; i++) {
        //name
        const BYTE* sdata = mem + (20 + (i - 1) * 30); //sample header data
    ...
    for (int pass = 0; pass <= nofpass; pass++) { //---TRANSITION 0/1---
        ...
        for (i = 0; i < songlen; i++) {
            ...
            const BYTE* pdata = mem + (patstart + patnum * patternsize); //beginning of the pattern
            ...
            for (m = ThisPatternFromRow; m < 64; m++) {
                for (n = 0; n < chnls; n++) {
                    const BYTE* bdata = pdata + m * chnls * 4 + n * 4;
            ...
            for (int ch = 0; ch < chnls; ch++) {
                const BYTE* tdata = pdata + ch * 4; //the beginning of the track data source
                ...
                for (sline = ThisPatternFromRow, dline = 0; sline < 64; sline++, dline++) {
                    const BYTE* bdata = tdata + sline * chnls * 4; //pointer to 4 bytes block
                    ...
                    TonePortamento:
                        if (x_portamento && tporon[ch]) { //the last time was portamento
                    ...
                    if (effect == 0x01) { //1xx	Portamento Up
                        ...
                        pspeed = param; //portamento speed
                        goto Effect3;
                    } else if (effect == 0x05) { //5xx	continue toneportamento (+ simultaneous volume slide, to work low)
                        pspeed = tporspeed[ch]; //takes over previous speed
                        goto Effect3; //same as effect 3, but without setting the portamento speed
                    } else if (effect == 0x03) { //3xx	TonePortamento
                        if (param) {
                            pspeed = tporspeed[ch] = param; //TONE portamento speed only for parameter 3xx
                        } else {
                            pspeed = tporspeed[ch]; //if 300 => continue portamento is the last used speed
                    ...
                    if (effect == 0x0a //Axx	volumeslide xx = 0x decrease, xx = x0 increase
                        ...
                        tr->speed[dline] = ss;
                    } else if (effect == 0x0d) { //Dxx	pattern break (xx = goto xx line in the next pattern <- hm, we do not know)
    ...
    if (m_songgo[dsline - 1] < 0) {
        m_songgo[dsline] = restartpos;
        dsline++;
    } //loop at the beginning or where it wants according to header[951]
    ...
    result.destnum = destnum;
    result.nonemptysamples = nonemptysamples;
    result.songlines = dsline;
