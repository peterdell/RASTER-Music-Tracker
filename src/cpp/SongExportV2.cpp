...
bool CSong::ExportV2(CSong& song, std::ofstream& ou, SongIOType iotype, LPCTSTR filename) {
    ...
    switch (iotype) {
    case SongIOType::RMT:
        return CRmtExporter::ExportAsRMT(song, ou, &exportDesc);
    case SongIOType::RMTSTRIPPED:
        return CRmtExporter::ExportAsStrippedRMT(song, ou, &exportDesc, filename);
    case SongIOType::ASM:
        return CASMFileExporter::ExportAsAsm(song, ou, &exportDesc);
    case SongIOType::ASM_RMTPLAYER:
        return CASMFileExporter::ExportAsRelocatableAsmForRmtPlayer(song, ou, &exportDesc);
    case SongIOType::SAPR:
        return songExporter.ExportSAP_R(songExport, ou);
    case SongIOType::LZSS:
        return songExporter.ExportLZSS(songExport, ou);
    case SongIOType::LZSS_SAP:
        return songExporter.ExportSAP_B_LZSS(songExport, ou);
    case SongIOType::LZSS_XEX:
        return songExporter.ExportXEX_LZSS(songExport, ou);
    case SongIOType::WAV:
        return songExporter.ExportWAV(songExport, ou, g_Pokey, g_Atari.GetMemoryAt(0));
    }
