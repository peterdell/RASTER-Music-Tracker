...
void CPokeyView::Draw(const CSong& song, const CTuning& tuning, const bool explorerMode, const CPokeyController& pokeyController, const CAtari& atari) {
    ...
    for (int pokey = 0; pokey < pokeyCount; pokey++) {
        ...
        const auto audf3Address = pokeyAddress + 0x4;

        const auto audctlAddress = pokeyAddress + 0x08;
        const auto skctlAddress = pokeyAddress + 0x0f;
        ...
        // Rows
        const auto pokeyBaseRow = ((pokey == 0) ? pokey1Row : pokey2Row);
        const auto channelBaseRow = pokeyBaseRow + 2;
        const auto audctlRow = channelBaseRow + 4;
        ...
        static auto constexpr POKEY_CHANNELS = 4;
        for (int pokeyChannel = 0; pokeyChannel < POKEY_CHANNELS; pokeyChannel++) {

            // Addresses
            const auto audfAddress = pokeyAddress + pokeyChannel * 2;
            ...
            if (pokeyChannel % 2 == 1) { // only in valid 16-bit channels
                const auto audfLowAddress = audfAddress - 2;
                audfLow = memory[audfLowAddress];
            ...
            const BOOL SAWTOOTH = (CH1_179 && CH3_179 && HPF_CH13 && (dist == 0xA0 || dist == 0xE0) && (pokeyChannel == 0)) ? 1 : 0;
            ...
            if ((JOIN_16BIT || JOIN_64KHZ || JOIN_15KHZ) && !volLow) { //16-bit without Reverse-16 output
