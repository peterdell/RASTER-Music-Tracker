...
#define Hook2(g1, g2)																				\
    ...
    if (g_active_ti == Part::PART_TRACKS) //bigger look for track edit mode
    {
        ...
        for (int channelNr = 0; channelNr < GetTracks(); channelNr++)
        {
            ...
            acol = CChannelControl::GetChannelOnOff(channelNr) ? ((audc & 0x10) ? CRGBColor::VOLUME_ONLY : CRGBColor::NORMAL) : CRGBColor::MUTE;

            // Check if its a two tone channel (1 or 5)
            if (CChannelControl::GetChannelOnOff(channelNr) && ((skctl1 == 0x8b && channelNr == 0) || (skctl2 == 0x8b && channelNr == 4))) acol = CRGBColor::TWO_TONE;
...
void CSong::DrawSong()
{
    ...
    for (i = 0; i < 4; i++, k += 24)
    {
        ...
        if (CChannelControl::GetChannelOnOff(i))
        {
...
void CSong::DrawTracks()
{
    ...
    for (i = 0; i < g_tracks4_8; i++, x += 16 * 8)
    {
        ...
        color = (CChannelControl::GetChannelOnOff(i)) ? TextColor::WHITE : TextColor::GRAY;	//channels off are in gray
