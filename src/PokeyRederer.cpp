...
// TODO: Move this to the InitSound
constexpr int CHANNELS = 2;
constexpr int BITRESOLUTION = 8;
constexpr int OUTPUTFREQ = 44100;
constexpr int LATENCY = 3; //3 Frames
...
CAtari::ClockFrequency FREQ_17(bool ntsc) {
    return CAtari::GetClockFrequency(ntsc);
}

int CYCLESPERSCREEN(bool ntsc) {
    return (int)(((float)FREQ_17(ntsc)) / FRAMERATE); // TODO: This is not the exacty framerate, so maybe that's why the frequencies are a littel off?
}

float CYCLESPERSAMPLE(bool ntsc) {
    return  ((float)FREQ_17(ntsc) / OUTPUTFREQ); // TODO Really float required?
}
...
BOOL CXPokey::RenderSound1_50(int instrspeed)
{
    ...
    for (; instrspeed > 0; instrspeed--)
    {
        ...
        {
            int cycles = (unsigned short)((float)renderpartsize / CHANNELS * CYCLESPERSAMPLE(g_ntsc));
            while (cycles > 0 && renderpartsize > 0)
            {
                // The maximum number of cycles that can be generated is CYCLESPERSCREEN
                auto cyclesPerScreen = CYCLESPERSCREEN(g_ntsc);
                int rencyc = (cycles > cyclesPerScreen ? cyclesPerScreen : cycles);
